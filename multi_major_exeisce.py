...
# ----------------------設定-------------------------
SLCdatapath = '/media/alt41450/SAR/Ascending_mix'  #影像資料路徑
...
table_allfile = df1.sort_values(["date"], ascending=True).reset_index(drop=True)
...
for i in range (0,1):
    for j in range(i,len(df.index)):
        ...
        #print(df["datecount"][j],df["datecount"][i],dayrange)
...
#print(timeselect)
unique_set = set(date2)
date2_uni = list(unique_set)


#print(date2)
print(date2_uni)
print('count:',len(date2_uni))
...
for i in date2_uni:
    ...
    for j in range(len(table_allfile.index)):
        if i[0:8]==table_allfile["date"][j]:
            list1date.append(table_allfile["filename"][j])
        if i[9:17]==table_allfile["date"][j]:
            list2date.append(table_allfile["filename"][j])
    dict_one2all1[str(i)] = list1date
    dict_one2all2[str(i)] = list2date
#print('a',dict_one2all1)
#print('b',dict_one2all2)
...
for i,j in zip(range(len(date2_uni)),date2_uni):
    
    os.mkdir('%s/AllTest/%s' %(outputpath,j) )
    if (Dem):
        shutil.copyfile('%s/reprj.dem.wgs84'%(outputpath), '%s/AllTest/%s/reprj.dem.wgs84' %(outputpath,j) )
        shutil.copyfile('%s/reprj.dem.wgs84.vrt' %(outputpath), '%s/AllTest/%s/reprj.dem.wgs84.vrt' %(outputpath,j) )
        shutil.copyfile('%s/reprj.dem.wgs84.xml' %(outputpath), '%s/AllTest/%s/reprj.dem.wgs84.xml' %(outputpath,j))
    #secondary
    ...
    change_node_text(text_nodes, "%s/%s,%s/%s,%s/%s" %(ASFdatapath,dict_one2all2[j][0],ASFdatapath,dict_one2all2[j][1],ASFdatapath,dict_one2all2[j][2]))
    
    write_xml(tree, "%s/AllTest/%s/secondary.xml" %(outputpath,j))
    #reference
    ...
    change_node_text(text_nodes2, "%s/%s,%s/%s,%s/%s" %(ASFdatapath,dict_one2all1[j][0],ASFdatapath,dict_one2all1[j][1],ASFdatapath,dict_one2all1[j][2]))
    
    write_xml(tree2, "%s/AllTest/%s/reference.xml" %(outputpath,j))
    #topsApp
    tree3 = read_xml("%s/topsApp.xml"%(outputpath))
    write_xml(tree3, "%s/AllTest/%s/topsApp.xml" %(outputpath,j))
...
error = []
for i in date2_uni:
    with cd("%s/AllTest/%s" %(outputpath,i)):
...
#空間基線(讀檔案)
Baselines = []
for i in range(0,int(len(date2_uni)/10+1)):
    a = date2_uni[i]
...
content = {
    "testFile": testFile,
    #"filename1": dict_one2all1[testFile]
    #"filename2": dict_one2all2[testFile]
    "Baselines": Baselines,
    
}

df22 = pd.DataFrame(content)
df2 = df22.reset_index(drop=True)#.sort_values(["Baselines"], ascending=True)  #test    
print(df2)   
...
for x,y in zip(df2new["testFile"], df2new["testFile"]):
    if os.path.exists('%s/AllTest/%s/merged/filt_topophase.unw.geo.vrt'%(outputpath,x)):
        ...
        with cd("%s/AllTest/%s/merged" %(outputpath,x)):
            #os.system("gdal_translate topophase.cor.geo -b 2 %s.geo.cc.tif"%(y))#band2
